...
MainWindow::MainWindow(int userId, QString userName, std::shared_ptr<Client> dbPtr, QWidget *parent):
    QMainWindow(parent),
    ...
    m_userName(userName)
{
    ...
    if(dbPtr) {
        ...
        m_dbPtr = std::make_shared<Client>();
    }
...
MainWindow *MainWindow::createClient(std::shared_ptr<Client> dbPtr)
{
    StartScreen s(dbPtr);
    auto result = s.exec();
